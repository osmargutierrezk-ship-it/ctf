...
function renderSidebar(activePage) {
  ...
  const tesoreríaLinks = [
    { href: '/dashboard.html',        icon: '📊', label: 'Dashboard' },
    { href: '/mis-solicitudes.html',  icon: '📥', label: 'Solicitudes' },
    { href: '/cambiar-password.html', icon: '🔒', label: 'Cambiar Contraseña' },
  ];

  const links = user.rol === 'TESORERIA' ? tesoreríaLinks : contadorLinks;
  const roleLabel = user.rol === 'TESORERIA' ? 'Tesorería' : 'Contador';
  const roleClass = user.rol === 'TESORERIA' ? 'role-tesoreria' : 'role-contador';
